...
static RtspRouteCtx_t *find_route_ctx(const AiispRunnerCfg_t *cfg,
...
int AiispRunner_Run(const AiispRunnerCfg_t *cfg, volatile bool *running)
{
    ...
    RtspRouteCtx_t route_ctxs[AIISP_RUNNER_MAX_RTSPS];
    int ret = -1;

    if (!cfg || !running) {
        printf("[AIISP_RUNNER] invalid input args\n");
        return -1;
    }

    if (cfg->rtsp_count <= 0 || cfg->rtsp_count > AIISP_RUNNER_MAX_RTSPS) {
        printf("[AIISP_RUNNER] invalid rtsp count: %d\n", cfg->rtsp_count);
        return -1;
    }
    ...
    if (!pipe) {
        printf("[AIISP_RUNNER] CamPipe_Create failed\n");
        return -1;
    }

    queue = DataQueue_Create(16, DATA_QUEUE_ITEM_MAX);
    if (!queue) {
        printf("[AIISP_RUNNER] DataQueue_Create failed\n");
        goto cleanup;
    }
    ...
    for (int i = 0; i < cfg->pipe_cfg.output_count; ++i) {
        ...
        if (CamPipe_RegisterFrameCallback(pipe, route_id,
            ...
            printf("[AIISP_RUNNER] register callback failed for route[%d]\n", route_id);
            goto cleanup;
        }
    }

    if (CamPipe_Start(pipe) != 0) {
        printf("[AIISP_RUNNER] CamPipe_Start failed\n");
        goto cleanup;
    }
    ...
    for (int i = 0; i < cfg->rtsp_count; ++i) {
        route_ctxs[i].server = RtspServer_Create(cfg->rtsps[i].rtsp_port,
                                                 ...
                                                 RTSP_SERVER_CODEC_H264);
        if (!route_ctxs[i].server) {
            printf("[AIISP_RUNNER] RtspServer_Create failed, port:%d, path:%s\n",
                   cfg->rtsps[i].rtsp_port, cfg->rtsps[i].rtsp_path);
            goto cleanup;
        }
        printf("[AIISP_RUNNER] route[%d] ready: rtsp://<board-ip>:%d%s\n",
            ...
            if (meta.magic == FRAME_META_MAGIC &&
                ...
                if (route_ctx && route_ctx->server) {
                    ...
                    if (cfg->fps > 0 && route_ctx->frame_count % (cfg->fps * 5) == 0) {
                        printf("[AIISP_RUNNER] route[%d] pts:%llu size:%u frames:%u\n",
