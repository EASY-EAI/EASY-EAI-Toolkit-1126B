...
static void print_usage(const RK_CHAR *name) {
	...
	printf("\t-J | --avs_json_path: AVS json config path for RV1126B blend mode, "
	       "e.g. /oem/usr/share/avs_json/rk_2_1920x1080.json\n");
	printf("\t                      When set, overrides calib/lut params (uses librkALG_avsCore.so)\n");
}
...
int main(int argc, char *argv[]) {
	...
	RK_BOOL bIfOsdDisplay = RK_FALSE;
	...
	if (pAvsJsonFilePath) {
		ctx->avs.stAvsGrpAttr.jsonPath = pAvsJsonFilePath;
	}
	/* set chn0 */
